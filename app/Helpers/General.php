...
{
    $folder = \App\Models\MailboxFolder::where('title', "Bandeja")->first();
...
{
    if(!$status)
        return \App\Models\Contact::all()->where('company_id', Auth::user()->company_id);

    return \App\Models\Contact::join('contact_status', 'contact_status.id', '=', 'contact.status')
        ->where('contact_status.name', $status)->where('company_id', Auth::user()->company_id)
        ->get();
...
{
    return \App\User::where('is_admin', 0)->where('is_active', 1)->where('company_id', \Illuminate\Support\Facades\Auth::user()->company_id)->orderByDesc('company_id')->get();
}
